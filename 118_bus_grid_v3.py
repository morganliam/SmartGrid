...
mal_throughputs = np.linspace(1, 5.00, 1000)
...
adistmal_list = np.zeros(shape=1000)
for i in range(1000):
    adistmal_list[i] = 3 + random.uniform(-.00025, .00025)


def adistmal():
    return random.choice(adistmal_list)
...
def adist():
    ...
    return random.choice(adist_list)
    ...
    for step in range(500):
        ...
        for i in range(len(ad_list)):
            ...
            if (ad_list[i][0] not in att_values) and (ad_list[i][1] not in att_values):
                ...
                inter_arrivals.append(max(ps.arrivals))
                p_count.append(pg.packets_sent)
                TD_list.append(max(ps.waits))
            ...
            if (ad_list[i][0] in att_values) or (ad_list[i][1] in att_values):
                ...
                # Create the packet generators and sink
                ps = PacketSink(env, debug=False, rec_arrivals=True, absolute_arrivals=False)
                pg = PacketGenerator(env, "Greg", adistmal, sdist)
                ...
                inter_arrivals.append(max(ps.arrivals))
                p_count.append(pg.packets_sent*10)
                TD_list.append(max(ps.waits))
                Bw_list.append(random.choice(mal_throughputs))
