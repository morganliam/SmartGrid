...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 17 06:22:49 2019
...
class PacketGenerator(object):
    ...
    def run(self):
        ...
        while self.env.now < self.finish:
            self.time_sent.append(self.env.now)
...
class PacketSink(object):
    ...
    def put(self, pkt):
        if not self.selector or self.selector(pkt):
            ...
            self.traverse_times.append(self.arrivals[self.packets_rec] - pkt.time)
            self.packets_rec += 1
            self.bytes_rec += pkt.size
            if self.debug:
#                print("now")   #same times as the packet arrivals with absolute arrival=true
#                print(self.env.now)
                print("packet time")
                print(pkt.time)
#                print("traverse times")  #same time as the packet sink waits when absolute arrival=true
#                print(self.traverse_times[self.packets_rec - 1])
